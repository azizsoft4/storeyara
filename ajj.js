...
if(window.innerWidth > 599){myf(0);}
for(let i=0;i<3;i++){document.getElementById('deta_'+i).innerHTML =d["deta"][i];}
}function myf(n){if(n>d['img'].length-1){n=0;}if(n<0){n=d['img'].length-1;}
document.getElementById('sh_f').src=d['img'][n];document.getElementById('vewerimg').src=d['img'][n];document.getElementById('sh_f').classList.add('o0');
addop('img',n);if(window.innerWidth < 600){myf2(n);}}
...
e1.innerHTML ="تمت إضافة المنتج لسلة التسوق";
...
x.send('a='+JSON.stringify(d)+'&n='+document.getElementById('noumcom').value+'&tail='+document.getElementById('vtail').value+'&color='+document.getElementById('vcolor').value);}
function che(yy){
let el=document.getElementById('noumcom');
if(Number.isInteger(Number(el.value)) && Number(el.value) > 0){
el.value=Number(el.value) + Number(yy);}else{el.value="1";}}
